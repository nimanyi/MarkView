...
│  ├─ layout.ts             # 视图模式（双栏 / 仅编辑 / 仅预览，Ctrl+Shift+V 循环）
│  ├─ settings.ts           # 设置面板（主题/视图/字号/同步滚动/工具栏/自动保存，即时生效并持久化）
...
│  ├─ tabs.ts               # 多标签页（路径 + 磁盘快照 + 选区/滚动现场，dirty 圆点）
│  ├─ contextmenu.ts        # 右键菜单（屏蔽 WebView 默认菜单，按位置自绘）
...
> 设置面板（Ctrl+,）：主题三选、编辑 / 预览字号（12~20px）、同步滚动与工具栏开关，即时生效并持久化；
> 自动保存：从文件打开的文档停止输入 2 秒后自动写回原文件（未命名文档跳过），设置面板可开关；
> 视图模式（Ctrl+Shift+V）：双栏 / 仅编辑 / 仅预览三态切换，设置面板可选并持久化，切回双栏自动恢复分栏比例；
> 隐藏 / 显示预览（Ctrl+Shift+D）：双态直切（仅编辑 ↔ 双栏），不必经过三态循环；
> 修复：工具栏隐藏后「展开」按钮因样式覆盖 hidden 属性而常驻、易被误点唤回的问题（全局 [hidden] 生效）；
> 右键菜单：屏蔽 WebView 默认菜单（查看源代码 / 检查 / 属性等开发项），换成按位置的剪切 / 复制 / 粘贴 / 全选 / 复制链接；发布构建拦截 Ctrl+U、Ctrl+R / F5；
> 多标签页：可同时打开多份文档，顶栏下方标签条点击切换，Ctrl+Tab / Ctrl+Shift+Tab 循环、Ctrl+W / 中键关闭；标签圆点示脏，打开新文件不打断当前编辑（旧文档连同修改留在后台），关闭脏标签或窗口时三态确认；
> 使用说明（Ctrl+Shift+H）：应用简介、界面组成与全部快捷键速查、版本与技术栈信息。
